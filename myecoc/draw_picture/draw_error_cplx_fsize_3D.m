%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% functon is to draw ensemble error,DC level and frequence changing with feature size
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        for dc_option = [1,2,3,4,5,8]
            ...
            for feature_option = 1:size(feature_size,2) %feature size                
                ...
                for dataset_option = 1: size(DS,2) %dataset
                    ...
                    %加载dcplx数据
                    matname=['E:\桌面\ecoc\ecoc-2.26\ICDC-ECOC\data\论文结果整理-期刊-pair\new_N3_F3\acc_dcplx_classifier\data_fs_',num2str(features),'\',FS{fs_option},'\dcplx\dcplx_',Learners{lindex},'_',DS{dataset_option},'.mat'];
                    load(matname);
                           
                    if dc_option == 8
                        dc_option = dc_option - 2;
                    end                    
            ...
            filepath = [genpath,'/data/论文结果整理-期刊-pair\new_N3_F3\error_cplx_fssize_3D\single_dataset\',FS{fs_option}];
            mkdir(filepath);
            if dc_option == 8
